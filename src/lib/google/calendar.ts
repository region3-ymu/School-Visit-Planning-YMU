// A small, dependency-free Google Calendar v3 client.
//
// Ported from the sibling YMU-A project (src/lib/google/calendar.ts), which
// runs it in both Next.js and Deno edge functions. Here it replaces the
// `googleapis` OAuth2 client: a service account sees every calendar that has
// been ACL-shared with it and subscribed to its calendarList, instead of only
// the calendars in one person's account, and it has no refresh token to expire
// when that person changes their password.
//
// Divergence from YMU-A: listEvents accepts `timeMax`, because this project
// syncs a bounded forward window rather than an entire calendar.
...
// ACCESS_TOKEN_SCOPE_INSUFFICIENT even though the caller already has real ACL
// read access to the calendar's data. calendar.calendarlist is a narrow scope
// granting only calendarList membership writes, not event data writes.
...
// TypeScript's DOM lib distinguishes ArrayBuffer from the wider ArrayBufferLike
// accepted by Uint8Array. Copying also gives WebCrypto a compact buffer.
...
async function isRateLimited(response: Response): Promise<boolean> {
  // Google reports quota exhaustion as a 403 with a specific error reason, not
  // a distinct HTTP status — a permissions 403 must not be retried.
  try {
...
// Shared retry/backoff for calendarList and events.list, used across the ~110
// calendars a full sync walks. Retries 429/5xx and rate-limited 403s with
...
export type ListEventsOptions = {
  ...
  syncToken?: string;
  pageToken?: string;
  /** Used only for the first/full sync. Ignored when syncToken is set. */
  timeMin?: string;
  /** Used only for the first/full sync. Ignored when syncToken is set. */
  timeMax?: string;
};
...
export class GoogleCalendarClient {
  ...
  async listEvents(options: ListEventsOptions): Promise<GoogleEventsPage> {
    ...
    // Google rejects timeMin/timeMax alongside syncToken. This conditional
    // makes the two modes impossible to accidentally mix.
  ...
  // Sharing a calendar with the service account (an ACL grant) gives it real
  // read access, but does NOT add the calendar to the service account's own
  // calendarList — and there is no UI for a service account to "accept" a
  // share the way a human would. Discovery below only sees calendarList
  // entries, so a newly shared calendar stays invisible until this call runs.
  // Idempotent: 409 means already subscribed, which is success.
...
// Pages through the full calendarList rather than trusting a single page — a
// service account subscribed to more than one page of calendars would
